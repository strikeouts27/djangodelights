...
urlpatterns = [
	...
	path("admin/", admin.site.urls), path("finance/", views.finance, name="finance"),
	...
	path("ingredients/add/", views.IngredientAdditionView.as_view(success_url="/ingredients/"), name="ingredientadd", ),
	...
	path("update/inventory/<int:pk>/", views.IngredientsListUpdateView.as_view(), name="updateinventory", ),
	path("item_details/<int:item_id>/", views.ItemDetails, name="item_details"),

	path("navbar/", views.navbar, name="navbar"), ]  # update view so the view had to be edited.}
